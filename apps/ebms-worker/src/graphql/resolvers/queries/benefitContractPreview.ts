...
export const benefitContractPreview: NonNullable<
  QueryResolvers<Ctx>["benefitContractPreview"]
> = async (_, args, ctx) => {
  ...
  const rows = await db
    ...
    .leftJoin(
      benefitRequests,
      and(
        eq(benefitRequests.benefitId, benefits.id),
        eq(benefitRequests.employeeId, actorEmployeeId),
      ),
    )
