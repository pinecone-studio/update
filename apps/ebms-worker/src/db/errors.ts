...
/** Detect if error is due to missing role_notifications table */
export function isRoleNotificationsTableMissing(error: unknown): boolean {
  const msg = String(error).toLowerCase();
  const cause =
    error &&
    typeof error === "object" &&
  ...
  const full = `${msg} ${cause}`;
  return (
    full.includes("role_notifications") &&
    (full.includes("no such table") || full.includes("no such column"))
  );
}
