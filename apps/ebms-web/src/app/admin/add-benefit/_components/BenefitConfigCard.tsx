...
const inputBase =
	"rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 dark:border-white/10 dark:bg-[#16142a] dark:text-white dark:placeholder:text-white/50 dark:focus:border-[#B18CFF]";
const labelClass = "block text-sm text-slate-600 mb-1 dark:text-[#A7B6D3]";
...
const EMPLOYEE_USAGE_PERIODS: {
	value: "7days" | "month" | "year" | "";
	label: string;
}[] = [
			...
			<div className="mt-3 flex flex-col gap-4 sm:flex-row sm:gap-6">
				<div className="flex min-w-0 flex-col gap-4">
					...
					<div className="w-full min-w-0 max-w-[220px]">
						<h3 className="text-sm font-medium text-slate-600 mb-1.5 dark:text-[#A7B6D3]">
							Financial Settings
						</h3>
						<div className="flex items-center gap-2">
							<input
								...
								className="flex-1 h-2 min-w-0 cursor-pointer rounded-lg bg-slate-200 accent-blue-500 dark:bg-[rgba(31,22,57,0.8)]"
							/>
							<div className="w-11 shrink-0 rounded border border-slate-200 bg-white px-2 py-1 text-center text-xs font-medium text-slate-900 dark:border-white/10 dark:bg-[#16142a] dark:text-white">
								{subsidy}%
							</div>
						</div>
					</div>
				...
				<div className="flex min-w-0 flex-col gap-4">
					<div className="flex flex-col gap-2">
						<div className="flex min-w-0 flex-col gap-1">
							<label className={labelClass}>Request Deadline</label>
							<input
								type="date"
				...
				<div className="flex min-w-0 flex-col gap-1">
					...
					<div className="flex min-w-0 items-center gap-2">
						...
						/>
						<span className="shrink-0 text-sm text-slate-600 dark:text-[#A7B6D3]">per</span>
						<select
							value={form.usageLimitPeriod ?? ""}
							onChange={(e) =>
								onChange({
									...form,
									usageLimitPeriod: (e.target.value || "") as
										| "7days"
										| "month"
										| "year"
										| "",
