...
export function PersonalDetailsSection({ name, id }: PersonalDetailsSectionProps) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-6 dark:border-[#243041] dark:bg-[#1A2333]">
      <div className="flex items-start justify-between gap-4">
        ...
        <button className="inline-flex items-center gap-2 rounded-lg bg-slate-200 px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-300 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700">
          <HiOutlinePencilSquare className="text-base" />
          Edit Profile
        </button>
      </div>
      <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
        <DetailRow icon={HiOutlineUserCircle} label="Full Name" value={name} />
        <DetailRow icon={HiOutlinePhone} label="Phone Number" value="—" />
        <DetailRow icon={HiOutlineEnvelope} label="Email Address" value="—" />
        <DetailRow icon={HiOutlineUserCircle} label="Employee ID" value={id} />
      </div>
    </section>
  );
}
