...
export function EmployeeListSection({ employees }: EmployeeListSectionProps) {
  return (
    <section className="flex min-h-[400px] flex-col overflow-hidden rounded-3xl dark:bg-[#20194D80]/50 sm:h-[500px]">
      <div className="grid grid-cols-[32px_1fr_auto] items-center gap-2 border-b border-white/10 px-3 py-3 text-left text-xs font-medium text-slate-500 dark:border-[#2C4264] dark:bg-[#60587B4D] dark:text-[#93A4C3] sm:grid-cols-[40px_1fr_1fr_auto] sm:gap-4 sm:px-6 sm:py-4 sm:text-sm">
        <span>#</span>
        <span>Employee</span>
        ...
        {employees.length === 0 ? (
          ...
          employees.map((emp, index) => (
            <Link
              ...
              className="-mx-3 grid grid-cols-[32px_1fr_auto] items-center gap-2 border-b border-white/5 px-3 py-3 text-left transition-all duration-300 ease-out last:border-b-0 hover:bg-white/5 dark:border-[#2C4264]/50 dark:hover:bg-white/5 animate-card-slide-in sm:-mx-6 sm:grid-cols-[40px_1fr_1fr_auto] sm:gap-4 sm:px-6"
              style={{
