...
export function BenefitModalDetails({
	...
	const summaryCopy = {
		...
		LOCKED: {
			title: "Locked",
			message:
				(benefit.lockReason ? translateLockReason(benefit.lockReason) : null) ||
		...
		<>
			...
			<div className="grid gap-x-6 gap-y-0 border-b border-slate-200 pb-4 md:grid-cols-2 dark:border-white/5">
				...
				(benefit.benefitStartDate || benefit.benefitEndDate) ? (
					<>
						<DetailCell label="Start" value={benefit.benefitStartDate || "-"} />
						<DetailCell
							label="End"
			...
			<div className="border-b border-slate-200 pb-4 dark:border-white/5">
				...
				>
					{rules.map((rule, index) => {
						...
						>
							<div className="grid h-[34px] w-[34px] shrink-0 place-items-center text-slate-700 dark:text-slate-300">
								{rule.passed ? (
									<FiCheck
										size={16}
							...
							<span
								className={`mt-1 inline-flex h-[34px] min-w-[92px] shrink-0 items-center justify-center rounded-[10px] border px-3 text-[14px] font-semibold tracking-[0.02em] ${
									rule.passed
										? `${theme.rulePass} text-emerald-800`
										: `${theme.ruleFail} text-rose-800`
								}`}
							>
