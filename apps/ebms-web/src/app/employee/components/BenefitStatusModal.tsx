...
import { MODAL_THEME, MODAL_THEME_LIGHT, getRules } from "./benefit-modal/theme";
...
export function BenefitStatusModal({
  ...
  const theme = isDark ? MODAL_THEME[benefit.status] : MODAL_THEME_LIGHT;
          ...
          >
            <div className="flex items-start justify-between gap-3 sm:gap-4">
              <div className="flex min-w-0 items-start gap-3">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[14px] border border-slate-200 bg-slate-100 text-slate-600 sm:h-14 sm:w-14 dark:border-white/15 dark:bg-white/5 dark:text-white/75">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[13px] font-normal leading-5 tracking-[-0.15px] text-slate-500 sm:text-[14px] dark:text-white/50">
              ...
              <button
                type="button"
                onClick={onClose}
                className={`grid h-10 w-10 place-items-center rounded-[14px] border transition sm:h-11 sm:w-11 ${theme.closeBtn}`}
                aria-label="Close"
              >
                <FiX size={20} />
              </button>
          ...
          {canRequest ? (
            <div className={`border-t px-3 py-3 sm:px-5 sm:py-4 ${theme.footer}`}>
              <button
                ...
                className="h-[50px] w-full rounded-2xl border border-blue-500 bg-blue-600 px-6 text-[15px] font-semibold text-white transition hover:bg-blue-700 sm:h-[56px] sm:text-[16px]"
              >
                Request benefit
              </button>
            </div>
