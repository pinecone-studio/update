...
export default function NotificationPage() {
  const notifications = [
    ...
    },
    {
      ...
      body:
        "Your Transit Pass benefit request has been approved. You’ll receive further details via email.",
      time: "1 day ago",
      tone: "success"
    },
    {
      ...
      body:
        "Your Health Insurance benefit expires in 30 days. Please review and renew if needed.",
      time: "2 days ago",
      tone: "warning"
    },
    {
      ...
      body:
        "You’ll become eligible for Gym Membership in approximately 15 days when your OKR score reaches 75%.",
      time: "3 days ago",
      tone: "info"
    }
  ]
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={`h-8 w-8 rounded-full bg-[#122033] grid place-items-center ${toneClasses}`}>
                    <HiOutlineInformationCircle className="text-lg" />
                  </div>
                  <div>
                    <p className="text-white text-sm font-semibold">{item.title}</p>
                    <p className="text-slate-400 text-xs mt-1">{item.body}</p>
                    <p className="text-slate-500 text-[10px] mt-2">{item.time}</p>
                  </div>
                </div>
