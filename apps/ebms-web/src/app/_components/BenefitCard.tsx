...
> = {
	ACTIVE: {
		card:
			"border-emerald-200 bg-white shadow-[var(--shadow-card)] dark:border-[#0FD0B7]/45 dark:bg-[radial-gradient(circle_at_82%_14%,rgba(103,168,255,0.34),transparent_34%),radial-gradient(circle_at_24%_22%,rgba(44,199,187,0.22),transparent_42%),linear-gradient(135deg,rgba(15,97,113,0.95)_0%,rgba(20,62,95,0.96)_52%,rgba(26,44,96,0.98)_100%)] dark:shadow-[0_10px_30px_rgba(28,106,175,0.32),0_0_0_1px_rgba(40,214,199,0.18)]",
		button:
			"border border-emerald-200 bg-emerald-50/80 text-emerald-800 hover:bg-emerald-100 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10",
		iconWrap:
			"border border-emerald-200 bg-emerald-100 dark:border-white/10 dark:bg-white/5",
		iconColor: "text-emerald-700 dark:text-white/70",
		sweep: "rgba(52,211,153,0.16)",
	},
	ELIGIBLE: {
		card:
			"border-blue-200 bg-white shadow-[var(--shadow-card)] dark:border-[#4A64FF]/40 dark:bg-[radial-gradient(circle_at_84%_16%,rgba(120,144,255,0.22),transparent_30%),linear-gradient(135deg,rgba(44,44,107,0.96),rgba(55,33,91,0.96))] dark:shadow-none",
		button:
			"border border-blue-200 bg-blue-50/80 text-blue-800 hover:bg-blue-100 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10",
		iconWrap:
			"border border-blue-200 bg-blue-100 dark:border-white/10 dark:bg-white/5",
		iconColor: "text-blue-700 dark:text-white/70",
		sweep: "rgba(96,165,250,0.16)",
	},
	PENDING: {
		card:
			"border-amber-200 bg-white shadow-[var(--shadow-card)] dark:border-[#C26E22]/40 dark:bg-[radial-gradient(circle_at_84%_16%,rgba(255,199,125,0.18),transparent_30%),linear-gradient(135deg,rgba(92,54,37,0.96),rgba(73,47,54,0.96))] dark:shadow-none",
		button:
			"border border-amber-200 bg-amber-50/80 text-amber-800 hover:bg-amber-100 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10",
		iconWrap:
			"border border-amber-200 bg-amber-100 dark:border-white/10 dark:bg-white/5",
		iconColor: "text-amber-700 dark:text-white/70",
		sweep: "rgba(251,146,60,0.16)",
	},
	LOCKED: {
		card:
			"border-rose-200 bg-white shadow-[var(--shadow-card)] dark:border-[#A23F82]/40 dark:bg-[radial-gradient(circle_at_84%_16%,rgba(225,117,232,0.18),transparent_30%),linear-gradient(135deg,rgba(76,33,89,0.96),rgba(66,27,68,0.96))] dark:shadow-none",
		button:
			"border border-rose-200 bg-rose-50/80 text-rose-800 hover:bg-rose-100 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10",
		iconWrap:
			"border border-rose-200 bg-rose-100 dark:border-white/10 dark:bg-white/5",
		iconColor: "text-rose-700 dark:text-white/70",
		sweep: "rgba(251,113,133,0.16)",
	},
	REJECTED: {
		card:
			"border-red-200 bg-white shadow-[var(--shadow-card)] dark:border-[#C54D58]/40 dark:bg-[radial-gradient(circle_at_84%_16%,rgba(230,126,146,0.18),transparent_30%),linear-gradient(135deg,rgba(93,39,55,0.96),rgba(73,28,39,0.96))] dark:shadow-none",
		button:
			"border border-red-200 bg-red-50/80 text-red-800 hover:bg-red-100 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10",
		iconWrap:
			"border border-red-200 bg-red-100 dark:border-white/10 dark:bg-white/5",
...
export const BenefitCard = ({
	...
	const cardClass = isAdminVariant
		? "border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] shadow-[0_12px_32px_rgba(15,23,42,0.08)]"
		: theme.card;
	const iconWrapClass = isAdminVariant
		? `${_iconBgColor} ${_iconColor} border border-black/5`
	...
	const labelClass = isAdminVariant
		...
		: "text-sm text-slate-500 dark:text-white/55";
	...
	const bodyClass = isAdminVariant
		...
		: "text-base text-slate-600 dark:text-white/85";
	return (
		<div
		...
		>
			...
			>
				{shouldAnimate && !isAdminVariant ? (
					<div
						className="pointer-events-none absolute inset-0 opacity-30 dark:opacity-70"
						style={{
				...
				<div className="pointer-events-none absolute inset-0 rounded-2xl bg-gradient-to-br from-slate-200/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100 dark:from-white/[0.02] dark:to-transparent" />
					...
					<div className="mb-3 flex items-start justify-between gap-3 sm:mb-4">
								...
								<div className="flex items-center gap-2">
									...
									(eligibilityRules ?? []).length > 0 ? (
										...
										>
											<FiInfo
												size={12}
												className="cursor-help text-slate-400 hover:text-slate-600 dark:text-white/40 dark:hover:text-white/70"
						...
						<div className="flex shrink-0 flex-col items-end gap-2">
							{hideStatusBadge ? null : (
								<div className="flex flex-col items-end gap-1">
									...
									{status === "PENDING" && pendingApprovalBy ? (
										<span
											className={`text-[10px] font-medium tracking-wide ${isAdminVariant ? "text-amber-700" : "text-amber-700 dark:text-amber-200/90"}`}
										>
							...
							{isAdminVariant && overrideApplied ? (
								<span className="inline-flex items-center rounded-full border border-amber-300/30 bg-amber-500/15 px-2.5 py-1 text-[10px] font-semibold tracking-[0.04em] text-amber-700 dark:text-amber-200">
									MANUAL OVERRIDE
								</span>
							) : null}
						</div>
					...
					{!isAdminVariant && !compact && status !== "ACTIVE" ? (
						<div className="mb-2">
							...
							<div className="mt-2.5 space-y-0.5">
								{subsidyPercentage ? (
									<div className="flex items-center justify-between gap-3 border-b border-slate-200 py-2 dark:border-white/10">
										<span className={labelClass}>Coverage</span>
										<span className={valueClass}>
											{subsidyPercentage} subsidy
										</span>
