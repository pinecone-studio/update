...
export default function VendorPaymentsPage() {
  ...
  return (
    <div className="space-y-8">
      <header>
        <h1 className="text-5 font-semibold text-white">Vendor Payments</h1>
      ...
      <section className="rounded-2xl border border-[#1E3258] bg-[#0D1B3A] p-6">
        <div className="flex items-center gap-3">
          ...
          <p className="text-5 text-slate-300">Total Pending Payments</p>
        </div>
        <p className="mt-3 text-5 font-bold text-white">$3,450</p>
      </section>
          ...
          <table className="min-w-full text-left text-5">
            ...
            <tbody>
              {vendorPayments.map((item) => (
                <tr key={`${item.vendor}-${item.benefit}`} className="border-b border-[#182A4A] last:border-b-0">
                  <td className="px-6 py-6 font-semibold text-white">{item.vendor}</td>
                  <td className="px-6 py-6 text-slate-300">{item.benefit}</td>
                  <td className="px-6 py-6 font-semibold text-white">{item.amount}</td>
      ...
      <section className="rounded-2xl border border-[#1E3258] bg-[#0D1B3A] p-6">
        <h2 className="text-5 font-semibold text-white">Recent Vendor Payments</h2>
        <div className="mt-5 space-y-4">
          {recentPayments.map((item) => (
            ...
            >
              <p className="text-5 text-slate-200">{item.vendor}</p>
              <div className="flex items-center gap-4">
                <p className="text-5 font-semibold text-white">{item.amount}</p>
